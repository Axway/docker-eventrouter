...
`
		var p5 yaml.Node
		err := yaml.Unmarshal([]byte(y3), &p5)
		if err != nil {
			t.Error("yaml.Node yaml unmarshall failed", err)
			return
		}
		err = YamlParseVerify("test", &TestZou{}, p5.Content[0])
		if err == nil {
			t.Error("yamlParseVerify: expecting error")
		}
		if !errors.Is(err, ErrFieldRequired) {
			t.Error("yamlParseVerify: expecting field required error")
		}
	}
